...
import { splitPdfFile, parseRangeString, SplitMode } from '@/lib/pdf/splitPdf';
...
export const SplitPdfWorkspace: React.FC<SplitPdfWorkspaceProps> = ({ onBack, t, lang }) => {
  ...
  const handleExecuteSplit = async () => {
    ...
    if (splitMode === 'custom') {
      try {
        ranges = parseRangeString(customRangeInput, pageCount);
        if (ranges.length === 0) {
          setErrorToast({
    ...
    try {
      ...
      setErrorToast({
        ...
        message: err?.message || 'Unexpected error while splitting PDF.',
      });
    }
  };
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="text-center space-y-2 py-2 max-w-lg mx-auto">
        <h2 className="text-2xl sm:text-3xl font-black tracking-tight text-slate-900 dark:text-white">
          {lang === 'en' ? 'Split PDF File' : 'Pisahkan Berkas PDF'}
        </h2>
        <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 font-medium leading-relaxed">
          {lang === 'en'
            ? 'Separate pages or extract specific ranges from your PDF document instantly.'
            : 'Pisahkan halaman atau ekstrak rentang tertentu dari dokumen PDF Anda secara instan.'}
        </p>
      </div>
      ...
      {!file ? (
        ...
        >
          <button
            type="button"
            className="px-8 py-4 rounded-2xl bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-white font-black text-base sm:text-lg shadow-lg shadow-amber-500/30 transition-all btn-press-effect flex items-center justify-center gap-2.5"
          >
