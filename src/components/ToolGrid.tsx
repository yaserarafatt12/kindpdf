    ...
    <div className="space-y-8 py-4">
      ...
      <div className="flex flex-wrap items-center justify-center gap-2">
        <button
          ...
          className={`px-4 py-2 rounded-full text-xs font-extrabold transition-all btn-press-effect border ${
            activeCategory === 'all'
              ? 'bg-slate-900 border-slate-900 dark:bg-white dark:border-white text-white dark:text-slate-900 shadow-md'
              : 'bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200/50 dark:hover:bg-slate-800'
          }`}
        >
          {t.allTools}
        </button>
      ...
      {/* Tool Cards Grid - Sharp Borders & Solid White Container */}
        ...
        {filteredTools.map((item) => (
          <div
            ...
            className={`p-6 rounded-3xl bg-white dark:bg-slate-900 border transition-all duration-200 flex flex-col justify-between gap-4 group ${
              item.isReady
                ? 'border-slate-300 dark:border-slate-800 hover:border-blue-600 dark:hover:border-sky-500 shadow-sm hover:shadow-xl cursor-pointer hover:-translate-y-1'
                : 'border-slate-200 dark:border-slate-800/80 opacity-70 cursor-not-allowed'
            }`}
          >
            {/* Header Icon & Badge */}
            <div className="flex items-center justify-between gap-2">
              <div className={`p-3.5 rounded-2xl ${item.iconBg} border border-slate-200/60 dark:border-slate-800 shadow-sm group-hover:scale-110 transition-transform`}>
                {item.icon}
              </div>
              <span
            ...
            {/* Footer Status */}
            <div className="pt-3 border-t border-slate-200/80 dark:border-slate-800 flex items-center justify-between text-[11px] font-semibold text-slate-500">
